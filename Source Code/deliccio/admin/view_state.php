...
else
{
	...
	{
		$sid = $_REQUEST["sid"]; 
		mysqli_query($conn, "DELETE FROM state WHERE state_id = $sid");
	...
	$sql = "SELECT * FROM state LIMIT $start_from, $num_rec_per_page"; 
           ...
           <?php
				...
				{?>
					<tr>
						...
						<td><a href="edit_state.php?sid=<?php echo $row['state_id']; ?>">Edit</a></td>
						<td><a href = "view_state.php?sid=<?php echo $row['state_id']; ?>" onclick="return confirmation();">Delete</a></td>
					</tr><?php
				}?>
        </tbody>
    </table>
...
<?php
	...
	echo "<a href='view_state.php?page=1'>".'|<'."</a> "; // Goto 1st page  

	for ($i=1; $i<=$total_pages; $i++) { 
				echo "<a href='view_state.php?page=".$i."'>".$i."</a> "; 
	}; 
	echo "<a href='view_state.php?page=$total_pages'>".'>|'."</a> "; // Goto last page
?>
</p>
