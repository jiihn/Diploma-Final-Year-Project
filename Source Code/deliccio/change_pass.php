...
<html>
<head>
<title>Lunar | Change Password</title>
<meta charset="utf-8">
...
<form name="changePassfrm" method="POST" action="">
		...
		<label>Old Password</label><br/>
			<input type="password" name="memberOldPass" maxlength="12" value="<?php echo isset($_POST["memberOldPass"]) ? $_POST["memberOldPass"] : ''; ?>" placeholder="Enter old password"/>
		...
		<div class="button">
		<input type="submit" name="changebtn" value="Change Password" />
		</div>
...
{
	$error = false;
	$oldPass = md5($_POST['memberOldPass']);
	$newPass = $_POST['memberNewPass'];
	$conNewPass = $_POST['ConMemberNewPass'];
	
	$sql = mysqli_query($conn, "SELECT member_password FROM member WHERE member_id = $sess_memid AND member_password = '$oldPass'");
			...
			if($reset)
			{
				...
				if($clear)
				{
					...
					echo "<script type=\"text/javascript\">
						setTimeout(\"location.href = 'profile.php';\",3500);
					</script>";
				}
			}
		}
