...
?>
	 ...
	 <div class="left-menu">
		...
		<ul>
			<?php
			$sql = mysqli_query($conn, "SELECT * FROM menutype");
				while($row = mysqli_fetch_assoc($sql))
				{
			?>
					<a href="view_menu.php?mt=<?php echo $row['menutype_id'] ?>"><li><?php echo $row['menutype_name']; ?></li></a>
			<?php
				}
			?>
		</ul>
	 ...
	 <div class="right-content">
		<?php
			$total = 0;
			$exists = false;
			$menu = mysqli_query($conn, "SELECT * FROM menu WHERE menutype_id = $mt AND isDeleted='No'");
			if(mysqli_num_rows($menu) == 0)
			{
				echo "<p>No menu in this menu type yet.</p>";
			}
			
			else
			{
				while($results = mysqli_fetch_assoc($menu))
				{
					$avg = 0;
					$total = 0;
					$exists = false;
	...
	?>	
	 ...
	 <div class="right-content">
		<?php
			...
			$avg = 0;
			$exists = false;
			$menu = mysqli_query($conn, "SELECT * FROM menu WHERE isDeleted='No'");
